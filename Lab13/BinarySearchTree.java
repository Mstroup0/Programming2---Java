...
// the use of the constructors from BinaryTreeBasis
  ...
  public boolean  duplicateCheck( BinarySearchTree t){

	return duplicateCheck(root, t(t.getRoot()));	
 ...
 private boolean duplicateCheck(TreeNode t1, TreeNode t2){
	...
	else if(t1.countNodes() > t2.countNodes() || t1.countNodes()< t2.countNodes(){
		return false;
	}
	else if(((Comparable)t1.getItem()).compareTo((Comparable)t2.getItem()) != 0)
		return false;
	else
	return true + (duplicateCheck(t1.getLeft(), t2.getLeft()) && duplicateCheck(t1.getRight(), t2.getRight()));
